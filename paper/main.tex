...
By comparing our ADL mechanisms to the one used in practice on Hyperliquid and Binance, we demonstrate empirically that Hyperliquid's production queue overutilized ADL by $\approx28\times$ relative to our optimal policy, imposing roughly \$653 million of unnecessary haircuts on winning traders.
...
\iparagraph{Empirical Results: The \$653 Million Inefficiency.}
...
We show that Hyperliquid autodeleveraged roughly $\sim$\$660m (simulated) to $\sim$\$2.1bn (realized) of winning trader PNL during the 12 minute cascade on October 10, 2025.

Over the same period, there were at least 40 ADL events which accumulated a total of $\sim$\$23.2m of true negative equity positions.\footnote{Hyperliquid's on-chain fill log for October 10, 2025 records 34{,}983 individual ADL executions across 19{,}337 distinct wallets and 162 tickers.
We analyze the per-fill data but aggregate those executions into the 40 queue waves discussed in the text.}
Measured shock-by-shock, this implies the production queue generated about $\sim$\$653M of overshoot, \ie~haircuts beyond the deficits that actually materialized.
This result highlights a massive capital inefficiency in current queue-based mechanisms: they are not just unfair, but they destroy hundreds of millions of dollars of trader value unnecessarily.
...
For example, if an exchange is already insolvent, $\mathsf{Solv}_T(\mathcal{P}_n) \leq 0$, then no deterministic ADL mechanism can return solvency.
To see this, we define four key quantifies, the total shortfall $D_T(\mathcal{P}_n)$, the maximum shortfall $\Delta_T(\mathcal{P}_n)$, the total winner's equity $W_T(\mathcal{P}_n)$, and the max-winner $\omega_T(\mathcal{P}_n)$:
\begin{align*}
...
W_T(\mathcal{P}_n) &= \sum_{\mathfrak{p} \in \mathcal{P}_n} e_T(\mathfrak{p})_+ \geq 0 &&& \omega_T(\mathcal{P}_n) &= \max_{\mathfrak{p} \in \mathcal{P}_n} e_T(\mathfrak{p})_+ \geq 0
\end{align*}
...
By definition, $\mathsf{Solv}_T(\mathcal{P}_n) = W_T(\mathcal{P}_n) - D_T(\mathcal{P}_n)$, so if $\mathsf{Solv}_T(\mathcal{P}_n) \leq 0$, then $D_T(\mathcal{P}_n) \geq W_T(\mathcal{P}_n)$, \ie~no amount of forced liquidations of positive equity can cover the shortfall.
...
An \emph{ADL policy} $\pi(\mathcal{P}_n)$ maps a perpetuals exchange to a fraction of shortfall to socialize, $\theta \in [0, 1]$, and a set of haircuts to apply to positions, $h \in [0,1]^n$.
...
We say an ADL policy $\pi$ if \emph{valid} if the following three constraints always hold:
\begin{equation}\label{eq:budget-balance}
\sum_{\mathfrak{p} \in \mathcal{P}_n} h_{\pi,i} e_T(\mathfrak{p})_+
= \theta_{\pi} D_T(\mathcal{P}_n)
= -\sum_{\mathfrak{p} \in \mathcal{P}_n} h_{\pi,i} e_T(\mathfrak{p})_-.
\end{equation}
\begin{equation}\label{eq:feasibility}
\theta_{\pi} D_T(\mathcal{P}_n) \leq W_T(\mathcal{P}_n).
\end{equation}
where $p_i$ is the $i$th trader's position.
The first two conditions~\eqref{eq:budget-balance} ensure that when an ADL policy is executed, only a $\theta$\% of the deficit is socialized from the winners to the losers (\ie~a budget balance constraint).
The third condition~\eqref{eq:feasibility} ensures that $\theta_{\pi}$ is chosen such that one does not need to haircut more equity than what the winners have (\ie~a feasibility constraint).
We define the post-policy deficit $D^{\pi}_T$, winners $W^{\pi}_T$, and max-winner $M^{\pi}_T$ as
\begin{align*}
D^{\pi}_T &= \theta_{\pi} D_T(\mathcal{P}_n) &&& \Delta^{\pi}_T &= \theta_{\pi} \Delta_T(\mathcal{P}_n) \\
W^{\pi}_T &= \sum_{\mathfrak{p} \in \mathcal{P}_n} (1-h_{\pi,i}) (e_T(\mathfrak{p}))_+ &&& \omega^{\pi}_T &= \max_{\mathfrak{p} \in \mathcal{P}_n} (1-h_{\pi,i}) (e_T(\mathfrak{p}))_+
\end{align*}
...
Given an ADL policy outputting $(\theta_{\pi}, h_{\pi})$, the positive equity positions $\mathcal{W}_T = \{ i : e_T(\mathfrak{p}_i) > 0\}$ are haircut by $h_{\pi, i}$, leading to equities $e_T(\mathfrak{p}_i) \leftarrow (1-h_{\pi, i})e_T(\mathfrak{p}_i)$ for $i \in \mathcal{W}_T$.
For instance, if we have equities $e_T = (10, 5, -5)$ and the ADL policy outputs haircuts $(0.5, 0, -1)$, then the post-ADL equities are $e_T = (5, 5, 0)$.
On the other hand, if the ADL policy outputs haircuts $h = (0.25, 0.5, -1)$, then the final equities after execution of the policy are $e_T = (7.5, 2.5, 0)$.
...
Given a haircut budget $B = \theta_{\pi_Q} D_T(\mathcal{P}_n)$, the algorithm to choose $h_i$ works as follows:
\begin{enumerate}
...
\item For $i \in [n]$, define $A_i = (B - \sum_{j=1}^{i} e_T(\mathfrak{p}_{\sigma(j)})_+)_+$, where $e_T(\mathfrak{p}_{\sigma(j)})$ is the equity of the $j$th largest position under the ranking $\sigma$
\item Define $h \in [0,1]^n$ as
\begin{equation}\label{eq:pl-ranking}
h_{\pi_Q,\sigma(i)} = \begin{cases}
    1 & \text{if } A_{i-1} - A_{i} = e_T(\mathfrak{p}_{\sigma(i)})_+,\; A_i > 0 \\
\frac{A_{i-1}}{e_T(\mathfrak{p}_\sigma(i))} & \text{if } A_i = 0, A_{i-1} > 0 \\
0 & \text{otherwise}
\end{cases}
\end{equation}
\end{enumerate}
Note this definition of $h_{\pi_Q}$ ensures that $\sum_{i=1}^n h_{\pi_Q,i} e_T(\mathfrak{p}_i) = B$, satisfying~\eqref{eq:budget-balance}.
On the other hand, since we only subtract positive equity positions (\eg~$j$ such that $e_T(\mathfrak{p}_{\sigma(j)})_+ > 0$), we satisfy~\eqref{eq:feasibility} by construction.
...
The Pro-Rata strategy, simply put, haircuts each winning user with positive equity by the same multiplicative fraction.
The fraction is subject to budget constraints, as we describe below.

Given a valid feasible shortfall $\theta_{\pi} D_T(\mathcal{P}_n \leq W_T(\mathcal{P}_n))$, the pro-rata haircut policy is defined as:
\begin{equation}\label{eq:pro-rata}
h_{\pi_{PR}}(\mathfrak{p}) = \frac{\theta_{\pi} D_T(\mathcal{P}_n)}{W_T(\mathcal{P}_n)}(e_T(\mathfrak{p}))_+
\end{equation}
...
h_{\pi_{LPR}}(\mathfrak{p}) = \left(\frac{\ell e_T(\mathfrak{p})_+}{\sum_{\mathfrak{p} \in \mathcal{P}_n} \ell e_T(\mathfrak{p})_+}\right) h_{\pi_{PR}}(\mathcal{P}_n)
\end{equation}
...
\end{equation}
This corresponds to an exchange guaranteeing that a user will never lose more than $\overline{h}_i$\% of their position in a single ADL round.
Moreover, an exchange might also offer an absolute guarantee to users: if your equity is positive and sufficiently large, you will never be have your equity cut to below some threshold $\underline{e}_i$.
This can be represented by the constraint 
\begin{equation}\label{eq:equity-constraint}
(1-h_{\pi, i})e_T(\mathfrak{p}_i) \geq \underline{e}_i
\end{equation}
...
Consider five accounts with equities $e_T = (10, 5, 1, -3, -12)$ and queue order $\sigma = (2, 1, 3, 5, 4)$ induced by the PNL‑leverage scores.
Let the queue policy choose severity $\theta_{\pi_Q} = \tfrac{1}{2}$, so the haircut budget is $B = \theta_{\pi_Q} D_T(\mathcal{P}_n) = 7.5$.
The construction in~\eqref{eq:pl-ranking} yields $A = (2.5, 0, 0, 0, 0)$ and haircuts $h = (0.25, 1, 0, 0, 0)$: the top winner is cut from $10$ to $7.5$, the second winner is fully wiped out, and the remaining accounts are unchanged, giving $e_T = (7.5, 0, 1, -3, -4.5)$.
If, before the next ADL decision, only the losing accounts partially recover to $e_T = (7.5, 0, 1, -1.5, -2.25)$, then a second round at the same severity $\theta_{\pi_Q} = \tfrac{1}{2}$ autodeleverages all negative‑equity accounts against the largest winner, yielding final equities $e_T = (3.75, 0, 1, 0, 0)$.
Under the pro‑rata rule~\eqref{eq:pro-rata} with the same severity $\theta_{\pi_{PR}} = \tfrac{1}{2}$, we have $D_T = 15$ and $W_T = 16$, so the uniform haircut factor is $\alpha = \theta_{\pi_{PR}} D_T / W_T = 15/32$ and each winning account’s equity is scaled by $(1-\alpha) = 17/32$, giving $e_T = (170/32, 85/32, 17/32, -3, -12)$.
...
\begin{enumerate}
  ...
  \item[\textbf{(F)}] \textbf{Fairness:} Moral hazard is bounded, \ie, $\omega^{\pi}_T/D^{\pi}_T = \Theta(1)$ and $\omega^{\pi}_T/\Delta^{\pi}_T = \Theta(1)$.
...
\begin{itemize}
  ...
  We analyze the ratio $\omega^{\pi}_T/D^{\pi}_T$ under heavy-tailed assumptions.
...
\begin{itemize}
    ...
    \[
    \mathsf{PTSR}_T(\pi) = \Expect\left[\frac{\omega^\pi_T}{D^\pi_T}\right].
    \]
    ...
    \[
    \mathsf{PMR}_T(\pi) = \Expect\left[\frac{\omega^\pi_T}{\Delta^\pi_T}\right].
    \]
...
Our asymptotic analysis relies on \emph{extreme--value scales}: deterministic sequences that characterize the typical magnitude of the largest winner equity and the largest loser shortfall in the limit of a large market.
...
\begin{equation}\label{eq:lln-scaling}
  \left(\frac{W_T(\mathcal{P}_n)}{n},\, \frac{D_T(\mathcal{P}_n)}{n}\right)
  \xrightarrow{p} (\mu_+,\mu_-).
\end{equation}
Here $\mu_+,\mu_->0$ summarize the average winner and loser magnitudes.
We define the respective maxima as
\begin{equation}\label{eq:maxima-def}
  \big(\omega_T,\, \Delta_T\big)
  = \left(
    \max_{i\in\mathcal{W}_T} \big(e_T(\mathfrak p_{i,T})\big)_+,\,
...
\begin{equation}\label{eq:ev-scaling}
  \left(\frac{\omega_T}{b^+_{k_n}},\, \frac{\Delta_T}{b^-_{m_n}}\right)
  \xrightarrow{p} (c_+,c_-).
\end{equation}
...
\begin{equation}\label{eq:quantile-scales}
  ...
  \right).
\end{equation}
where $F_+$ and $F_-$ denote the distribution functions of winner-side positive equities and loser-side shortfalls, respectively.

\iparagraph{Extreme–value severity scales.}
We refer to $\theta_n = \Theta(b_n/n)$ as the \emph{extreme--value severity scale}.
This choice aligns the haircut magnitude with the typical largest winner equity $b_n=b^+_{k_n}$ in a large market where $k_n=\Theta(n)$.
...
\begin{itemize}
  ...
  \]
  \item \emph{Power--law tails (Pareto/Fréchet).} For distributions satisfying $\Prob\{(e_T)_+>x\}\sim C_+x^{-\alpha_+}$ and $\Prob\{(-(e_T))_+>x\}\sim C_-x^{-\alpha_-}$ with $\alpha_\pm>0$,
  \[
...
\[
  H(\pi) \;=\; \sum_i h_{\pi,i}\,e_T(\mathfrak p_i)_+ \;=\; \theta_\pi D_T(\mathcal{P}_n),
\]
...
Conversely, fairness and revenue functionals depend on the winner-side post-ADL equities $\{(1-h_{\pi,i})e_T(\mathfrak p_i)_+\}_i$ and are invariant to the choice of $\theta_t$ once the total budget $H_t$ is fixed.
...
Suppose the top winner's equity scales sub-linearly, \eg~$b_n \sim c \log n$ (light tails).
...
As $n \to \infty$, this linear haircut cost overwhelms the logarithmic winner equity:
\[
e_n^{\text{post}} \approx (c \log n - \bar{\theta} \mu n)_+ \to 0.
\]
Thus, to preserve any winner profit, severity must vanish at the rate $\theta_n = O(b_n/n)$.
...
In our model, socialization is instantaneously zero‑sum: every dollar of deficit $D_T$ covered by ADL must come from the winners' equity.
If ADL usage does not vanish as the market grows larger ($n\rightarrow\infty$), then one cannot simultaneously keep deficits small (solvency, low $D_T$) and preserve winner payoffs (high $\omega_T$). 
...
Suppose that the extreme value scale of the winning trader's equity, $\omega_T^{\pi}(\mathcal{P}_n)$, is $b_n$.
Then, 
\[
\mathsf{PTSR}_T(\mathcal{P}_n, \pi) =  \Expect_{\pi}\left[\frac{\omega^{\pi}_T(\mathcal{P}_n)}{D^{\pi}_T(\mathcal{P}_n)}\right] \asymp \frac{b_n}{\theta_n\, n}
\]
In particular, the example above shows the order $b_n/n$ is tight: unless the severity vanishes at the extreme–value scale, $\theta_n=\Theta(b_n/n)$, one cannot preserve the best trader's profits as $n\to\infty$.
\end{proposition}

\iparagraph{Example.}
The bound $b_n/n$ dictates the maximal safe severity for different distributions:
\begin{itemize}
    \item \emph{Gaussian Winners (Light Tails):} For $e_i \sim \mathcal{N}(0,1)$, the maximum scales as $b_n \sim \sqrt{2\log n}$. 
    Thus, severity must vanish rapidly, $\theta_n \lesssim \sqrt{2\log n}/n$, to avoid autodeleveraging the top winner.
    This implies that the shortfall that can be covered goes to zero as $n\rightarrow \infty$.
    \item \emph{Pareto Winners (Heavy Tails):} For $e_i \sim \text{Pareto}(\alpha)$ with $\alpha > 1$, the maximum scales as $b_n \sim n^{1/\alpha}$. 
    Here, the constraint is looser but still vanishing as $n\rightarrow \infty$: $\theta_n \lesssim n^{-(1-1/\alpha)}$.
\end{itemize}
Even in the heavy-tailed case (infinite variance, $\alpha < 2$), budget balance implies that a constant fraction severity $\theta$ wipes out the top winner almost surely as $n \to \infty$.
...
Let $b_n^+$ (resp.\ $b_n^-$) denote the extreme value (EV) scales of winner and loser equities, respectively.
Aggregate shortfall scales with loser liability, $D_T \approx \ell^- b_n^-$, while the top winner's equity scales as $\omega_T \approx \ell^+ b_n^+$.
...
\[
\frac{H_n}{\omega_T} \approx \theta_n \frac{\ell^-}{\ell^+} \frac{b_n^-}{b_n^+}.
\]
If $\ell^- \gg \ell^+$, even a small severity $\theta_n$ creates a haircut larger than the top winner's entire position.
...
Let $b_n^+, b_n^-$ be the extreme value scales of the winning and losing traders, respectively.
...
\[
\Expect_{\pi}\!\left[\frac{\omega^{\pi}_T(\mathcal{P}_n)}{D^{\pi}_T(\mathcal{P}_n)}\right]\ \asymp\ \frac{\ell_n^+}{\ell_n^-}\cdot\frac{b_n^+}{b_n^-}\cdot\frac{1}{\theta_n}.
\]
\end{proposition}
...
Tail shocks necessarily haircut the top winners (Proposition~\ref{prop:ev-scale-severity-informal}), and high loser leverage magnifies that hit (Proposition~\ref{prop:excessive-leverage}).
...
PTSR/PMR play the same role as VGH fairness/solvency gauges in CCP recovery \citep{CPMI_IOSCO_2014,Gregory2015,DuffieZhu2011}.

\subsection{Queue-based Methods are the worst ADL policies for the top winning trade}\label{subsec:queue-worst}
One result that we see empirically in~\S\ref{sec:numerics} is that the Queue mechanism is by far the worst ADL policy for the top winning trade.
...
Among all valid ADL policies with fixed severity $\theta$, the \emph{Queue} policy (haircut largest winners first) uniquely minimizes the survival of the top winner.
Let $\omega_T^{\text{Queue}}$ and $\omega_T^{\text{PR}}$ be the top winner's post-ADL equity under Queue and Pro-Rata, respectively.
Whenever the haircut budget $H = \theta D_T$ satisfies $H \le e_{(1)}$, we have the strict gap
\[
\omega_T^{\text{PR}} - \omega_T^{\text{Queue}} \;=\; H \left(1 - \frac{e_{(1)}}{W_T}\right) \;>\; 0.
\]
Consequently, Pro-Rata strictly dominates Queue in terms of fairness metrics:
\[
\mathsf{PTSR}_T(\text{PR}) - \mathsf{PTSR}_T(\text{Queue}) \;=\; \theta\left(1 - \frac{e_{(1)}}{W_T}\right).
\]
...
Appendix~\ref{app:classical-risk-measures} further strengthens this statement by proving that Queue also minimizes the VaR and ES of the top survivor at every confidence level, so even classical risk metrics rank Queue as the worst policy for the top winner.
...
Combining haircut constraints~\eqref{eq:haircut-constraint} and equity constraints~\eqref{eq:equity-constraint}, we define an effective cap $\beta_i = \min\{\overline{h}_i, 1 - \underline{e}_i/e_T(\mathfrak{p}_i)\}$.
This $\beta_i$ is the most we can haircut account $i$ while honoring both its per-account haircut limit and its minimum post-ADL equity.
The maximum haircut capacity is 
\begin{equation}\label{eq:max-cap}
C(\beta) = \sum_{i=1}^n e_T(\mathfrak{p}_i) \beta_i.
\end{equation}
...
\sum_{i=1}^n h_{\pi_{CP}, i} e_T(\mathfrak{p}_i)_+ = \theta_{\pi} D_T(\mathcal{P}_n).
\end{equation}
...
\begin{itemize}
    ...
    \item \emph{Monotonicity (Stability):} The policy preserves the relative ordering of post-haircut equity. If $e_i \ge e_j$, then $(1-h_i)e_i \ge (1-h_j)e_j$.
\end{itemize}
...
The RAP rule allocates haircuts using weights $w_i = \lambda_{T,i} g(\lambda_{T,i})$.
With per-account caps $\beta_i$, the haircuts are:
\begin{equation}\label{eq:glpr}
    h_{\mathrm{RAP}(g), i} = \min\{\beta_i, \tau w_i\},
\end{equation}
where $\tau$ is calibrated to meet the target budget.
...
Intuitively, an optimal risk model should maximize the marginal reduction of $\delta_T$ per unit of equity $e_{T,i}$.
Differentiating $\delta_T$ with respect to the haircut $h_{T,i}$ reveals:
\begin{equation}\label{eq:delta-t-grad}
-\frac{\partial\,\delta_T(h)}{\partial h_{T,i}}
...
The greedy rule that maximizes the marginal reduction per equity-dollar is exactly the rule that prioritizes winners with the largest $\rho_i(\lambda_{T,i})$.
...
Thus, maximizing $\rho_i$ is equivalent to maximizing the net solvency gain per unit of equity.
For example, if a trader with leverage $\lambda=10$ faces an insurance cost of 0.5\% for a yield of 8\%, the net solvency gain is $0.08 - 10(0.005) = 0.03$ per unit equity.
...
\section{Empirical Analysis: The October 10 Event}\label{sec:empirical-oct10}\label{sec:numerics}

We evaluate the proposed ADL mechanisms using a high-fidelity replay\footnote{The author profusely thanks Mauricio Trujillo (\href{https://x.com/ConejoCapital}{@conejocapital}) and Hydromancer for providing, cleaning, and verifying this data.} of the October 10, 2025 liquidation cascade on Hyperliquid~\cite{CoinDesk2025LargestLiquidations}.
We use this replay to stress-test the ADL Trilemma (\S\ref{sec:adl-trilemma}), quantify fairness via PTSR/PMR (\S\ref{sec:adl-trilemma} and Figure~\ref{fig:emp-ptsr-pmr}), and benchmark RAP and other static baselines (\S\ref{sec:glpr}).

\subsection{Experimental Setup}

\paragraph{Dataset.}
We utilize the canonical reconstruction of the event~\cite{HyperMultiAssetedADL}, covering a 12-minute window (21:16--21:27 UTC) with \$2,103,111,431 in liquidations across roughly 160 assets.
The replay comprises $T=1097$ ADL shocks (201 with $D_t>0$).
Each shock records the realized deficit $D_t$ (total negative equity of the losers) and the winner-equity vector $e^\mathcal{W}_t=(e_{t,1},\dots,e_{t,|\mathcal{W}_t|})$.
The winner equity vector gives us the \emph{maximum haircut mass}, $W(e^{\mathcal{W}}_t) = \sum_{i=1}^{|\mathcal{W}_t|} e_{t,i}$.
This is the maximum profit that could be seized to cover a deficit.

Across the 201 shocks with $D_t>0$, the realized shortfall sums to $\sum_t D_t = \$23{,}245{,}151.37$.
The feasible haircut mass $\sum_t \min\{D_t, W(e^{\mathcal{W}}_t)\}$ is only \$6{,}620{,}946.51.
Consequently at least \$16.6M of the realized deficit is structurally uncovered regardless of policy, establishing a hard lower bound on cumulative residuals.

As an explicit example, consider the asset \textsc{WLD} on Hyperliquid.
There was a realized deficit of $D_t = \$11{,}558{,}884.49$ despite having a maximum haircut $W(e^{\mathcal{W}}_t) = \$2{,}507.11$. 
This means that every ADL strategy exits that round with \$11{,}556{,}377.38 of residual bad debt.
We note that the dataset provides the exact sequence of exogenous price shocks and trader positions, allowing for a counterfactual analysis of ADL responses holding market conditions fixed.

\paragraph{Policies.}
We compare the static benchmark $\pi_{\text{queue}}$ against the dynamic controllers defined in Appendix~\ref{app:strategies}:
\begin{itemize}
    \item \emph{Static Priority ($\pi_{\text{queue}}, \pi_{\text{smart}}$):} The standard greedy mechanism (Queue) and its deficit-capped variant (Smart Queue). $\pi_{\text{smart}}$ enforces $H_t \le D_t$ but retains the greedy priority ranking.
    \item \emph{Dynamic Severity ($\pi_{\text{exp}}, \pi_{\text{md}}$):} The Exponential Backoff and Mirror Descent controllers that optimize the scalar severity $\theta_t$ while allocating haircuts pro-rata.
    \item \emph{Vector Optimization ($\pi_{\text{vec}}$):} The full allocation policy minimizing the composite loss $L_t$ via high-dimensional mirror descent.
    \item \emph{Hybrid Blend ($\pi_{\text{hyb}}$):} A convex combination of the production Queue and optimized Levered Pro-Rata allocations (20\%/80\% mix). This approximates the recommended pairing of queue-style guardrails with pro-rata fairness without retraining the live mechanism.
\end{itemize}
...
\paragraph{Overshoot and Efficiency.}
We measure \emph{overshoot} on each shock as $(H_t-D_t)_+$, the haircuts applied beyond the realized deficit $D_t$.
Recall that for our optimized algorithms, we enforce the feasibility constraint~\eqref{eq:feasibility}, so $H_t \le D_t$.
This means that most of the strategies will have very low if not zero overshoot.\footnote{The non-zero overshoot is due to the fact that the strategies are generally local gradient steps, which accumulate some noise and error in how well they enforce constraints.}

The production queue $\pi_{\text{queue}}$ fully liquidates accounts and ignores feasibility, accumulating \$653.6M of overshoot with a \$47.1M single-shock spike.
All other controllers keep overshoot negligible: $\pi_{\text{smart}}$ is exactly capped with $H_t = D_t$; $\pi_{\text{md}}$ and $\pi_{\text{dyn2}}$ have maxima of \$89{,}537.21 and \$268{,}670.09, respectively; $\pi_{\text{exp}}$ peaks at \$223{,}886.86; the vector optimizer peaks at \$974{,}644.37.
We also study the effect of the hybrid policy $\pi_{\text{hyb}}$ which trades solvency for revenue smoothly.
This strategy reduces queue overshoot by $\approx 80\%$ (to \$134.4M total, \$9.4M max) while retaining much of the revenue retention of the dynamic controllers.

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/overshoot_time_series_with_cumulative.png}{\includegraphics[width=0.92\linewidth,height=0.35\textheight,keepaspectratio]{adl-simulation-try3/results/overshoot_time_series_with_cumulative.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{ADL overshoot ($H_t - D_t$). Dashed: per-shock overshoot. Solid: cumulative overshoot. The static policy $\pi_{\text{queue}}$ accumulates $\approx\$653$M of excess haircutting. Dynamic policies ($\pi_{\text{md}}, \pi_{\text{vec}}$) keep overshoot negligible by construction, while $\pi_{\text{exp}}$ maintains it within a bounded constant factor.}
  \label{fig:emp-overshoot}
\end{figure}

\paragraph{The ADL Trilemma.}
We analyze the ADL trilemma (Proposition~\ref{prop:adl-trilemma}) empirically by reporting, for each policy, its solvency (residual shortfall), fairness (PTSR/PMR), and revenue (fee-retention) metrics.
These three objectives summarize the trade-offs that each ADL policy makes.

Figure~\ref{fig:emp-regret-queue} anchors the ``solvency-at-any-cost'' corner of the trilemma: $\pi_{\text{queue}}$ slashes residual deficits (\ie~higher solvency) yet incurs runaway overshoot and fairness penalties (\ie~lower fairness and revenue).
Figure~\ref{fig:emp-regret-smart} shows that capping overshoot via $\pi_{\text{smart}}$ still leaves concentrated losses (\ie~low fairness) and elevated residual deficits once churn feedback kicks in (\ie~lower long term revenue).
Figures~\ref{fig:emp-regret-mirror} and~\ref{fig:emp-regret-dyn2} illustrate how dynamic mirror-descent controllers match the queue's residual deficits without its welfare cost, \ie~they have better revenue preserving properties than Queue while preserving approximately the same amount of solvency.

On the other hand, Figure~\ref{fig:emp-regret-backoff} demonstrates that (static) exponential backoff strategies can be competitive with dynamic policies if the deficit is sufficiently bounded.
By jointly optimizing allocation and severity, Figure~\ref{fig:emp-regret-vector} shows $\pi_{\text{vec}}$ delivering the fairest loss sharing and the steepest long term revenue recovery.
Finally, the hybrid $\pi_{\text{hyb}}$ in Figure~\ref{fig:emp-regret-hybrid} blends the production queue with Levered Pro-Rata, eliminating $\approx 80\%$ of Queue's overshoot and cutting revenue loss by nearly an order of magnitude while keeping residual deficits low.

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/deficit_flow/outstanding_overlay.png}{\includegraphics[width=0.92\linewidth]{adl-simulation-try3/results/deficit_flow/outstanding_overlay.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Outstanding negative equity (cumulative residual deficits). Lower is better. Despite its aggression, $\pi_{\text{queue}}$ fails to clear the deficit effectively due to feedback loops. $\pi_{\text{exp}}$ achieves competitive solvency without the welfare cost.}
  \label{fig:emp-outstanding}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/top_winner_haircut_time_series.png}{\includegraphics[width=0.92\linewidth]{adl-simulation-try3/results/top_winner_haircut_time_series.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Largest winner haircut (USD). $\pi_{\text{queue}}$ places a massive, spikey burden on single participants. Dynamic policies ($\pi_{\text{md}}, \pi_{\text{exp}}$) distribute the load, respecting fairness constraints.}
  \label{fig:emp-topwinner}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/ptsr_pmr_time_series.png}{\includegraphics[width=0.92\linewidth]{adl-simulation-try3/results/ptsr_pmr_time_series.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{PTSR (top) and PMR (bottom) on log scale. $\pi_{\text{queue}}$ exhibits ratios $>10^6$, indicating extreme moral hazard. Adaptive policies remain within the theoretical corridor ($\approx 1$--$10$).}
  \label{fig:emp-ptsr-pmr}
\end{figure}

\paragraph{Regret Decomposition.}
Figure~\ref{fig:emp-regret-queue} makes clear that overshoot (orange) and fairness (red) dominate the queue baseline.
Figure~\ref{fig:emp-regret-smart} cuts the overshoot bar almost entirely but replaces it with a larger residual (blue) mass as liquidity flees.
Mirror-descent controllers in Figures~\ref{fig:emp-regret-mirror} and~\ref{fig:emp-regret-dyn2} shrink every component simultaneously by aligning severity with observed deficits, with the warm-started variant tightening the residual tail.
Figure~\ref{fig:emp-regret-backoff} trades a small overshoot allowance for even lower fairness cost, while Figure~\ref{fig:emp-regret-vector} compresses all three components and traces the Pareto frontier across solvency, fairness, and revenue.

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/regret_components_queue.png}{\includegraphics[width=0.75\linewidth]{adl-simulation-try3/results/regret_components_queue.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Regret decomposition for the production Queue baseline $\pi_{\text{queue}}$. Overshoot (orange) and fairness (red) penalties dominate, illustrating static inefficiency.}
  \label{fig:emp-regret-queue}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/regret_components_smart_queue.png}{\includegraphics[width=0.75\linewidth]{adl-simulation-try3/results/regret_components_smart_queue.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Regret decomposition for the Smart Queue $\pi_{\text{smart}}$. Capping overshoot helps, but churn-driven residual deficits and fairness costs still dominate regret.}
  \label{fig:emp-regret-smart}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/regret_components_mirror.png}{\includegraphics[width=0.75\linewidth]{adl-simulation-try3/results/regret_components_mirror.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Regret decomposition for Mirror Descent $\pi_{\text{md}}$. Adaptive severity minimizes overshoot while balancing fairness and residual losses.}
  \label{fig:emp-regret-mirror}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/regret_components_dyn2.png}{\includegraphics[width=0.75\linewidth]{adl-simulation-try3/results/regret_components_dyn2.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Regret decomposition for the Dynamic2 controller $\pi_{\text{dyn2}}$. Warm-started mirror descent keeps overshoot tame while moderating fairness penalties.}
  \label{fig:emp-regret-dyn2}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/regret_components_backoff.png}{\includegraphics[width=0.75\linewidth]{adl-simulation-try3/results/regret_components_backoff.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Regret decomposition for Exponential Backoff $\pi_{\text{exp}}$. Feedback-tuned severity contains overshoot without sacrificing solvency or fairness.}
  \label{fig:emp-regret-backoff}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/regret_components_vector.png}{\includegraphics[width=0.75\linewidth]{adl-simulation-try3/results/regret_components_vector.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Regret decomposition for Vector Mirror $\pi_{\text{vec}}$. Joint severity and allocation updates compress every component, tracing the Pareto frontier.}
  \label{fig:emp-regret-vector}
\end{figure}

\begin{figure}[t]
  \centering
  \IfFileExists{adl-simulation-try3/results/regret_components_hybrid.png}{\includegraphics[width=0.75\linewidth]{adl-simulation-try3/results/regret_components_hybrid.png}}{\rule{0pt}{2in}\rule{0.9\linewidth}{0pt}}
  \caption{Regret decomposition for the Hybrid blend $\pi_{\text{hyb}}$ (20\% Queue, 80\% Levered Pro-Rata). Mixing the production policy with optimized allocations slashes overshoot and revenue loss while keeping residual deficits comparable to Queue.}
  \label{fig:emp-regret-hybrid}
\end{figure}

\subsection{Interpretation}
Taken together, the replay shows the production Queue is behaving exactly as the unbounded PoA story predicts: it buys solvency with overshoot and churn, and pays with fairness and future fees.
Policies that separate “how much” ($\theta_t$) from “who” ($h_t$) --- \eg~Exponential Backoff ($\pi_{\text{exp}}$) or Mirror Descent ($\pi_{\text{md}}$) --- land in a strictly better region of the trilemma: they keep solvency comparable while lifting PTSR/PMR and retaining revenue.
The convex hybrid $\pi_{\text{hyb}}$ makes this trade-off explicit: a simple 20\%/80\% mix of Queue and Levered Pro-Rata inherits most of the fairness and revenue gains of the vector policy without giving up the familiar solvency profile.

Based on these results, we offer three concrete prescriptions for Hyperliquid and similar venues:
\begin{enumerate}
\item \emph{Decouple severity from allocation.} Use a scalar feedback rule (\eg~$\pi_{\text{exp}}$ or $\pi_{\text{md}}$) to set $\theta_t$ from the realized deficit $D_t$, then allocate separately.
\item \emph{Adopt levered pro-rata.} Replace the greedy queue with Levered Pro-Rata (Eq.~\ref{eq:lev-pro-rata}) or the optimized vector $h_t$; this keeps PTSR/PMR near 1 and aligns haircuts with capacity.
\item \emph{Blend for transparency.} A light hybrid (20\% Queue, 80\% LPR) preserves the familiar queue feel while cutting overshoot by $\approx 80\%$ and recovering most of the revenue and fairness gains.
\end{enumerate}
...
We first that a negative result: it is impossible for a perpetuals to simultaneouslyoptimize for exchange solvency, long term revenue, and fairness to traders. 
...
The author would also like to thank Yi Sun, Anirudh Pai, Kshitij Kulkarni, Theo Diamandis, Matheus V. X. Ferreira, Guillermo Angeris, Victor Xu, and Vinayak Kurup for helpful discussions.
...
\begin{itemize}
  \item Shocks are indexed by $t=1,2,\dots,T$; winners $W_t$ with equities $e_{t,i}>0$ and losers $L_t$.
  \item Deficit $D_t\ge 0$; severity $\theta_t\ge 0$; haircut vector $h_t\in[0,1]^{|W_t|}$ with survivors $s_{t,i}=(1-h_{t,i})e_{t,i}$.
  \item Per–round budget $B_t=\min\{\theta_t D_t,\ \sum_i w_{t,i}\}$ for severity policies and $B_t=w_t^\top h_t$ for vector policies; $B_t^\star=\min\{D_t,\sum_i w_{t,i}\}$.
...
\input{liquidation-mechanics.tex}
\input{moral-hazard-2.tex}
\input{capped-pro-rata.tex}
\input{algorithms.tex}
\subfile{rap.tex}
\input{stackelberg.tex}
\input{smoothness.tex}
\input{empirical.tex}
\input{adl-trilemma-proof.tex}
