...
\]
the capped reverse-\-waterfilling with weights $w_{t,i}=\rho(\lambda_{t,i})=\lambda_{t,i}\psi_i(1/\lambda_{t,i})$ minimizes $\delta_t(h)$ among all $h$ with $\sum_i e_{t,i}h_{t,i}=b_t$ and $0\le h_{t,i}\le H_{t,i}$.
\end{proposition}
\begin{proof}
...
Maximizing $\sum_i \rho(\lambda_{t,i})\,e_{t,i} h_{t,i}$ under $\sum_i s_{t,i}=b_t$ and $0\le s_{t,i}\le e_{t,i}H_{t,i}$ is a fractional knapsack problem solved by sorting the values $\rho(\lambda_{t,i})$.
The optimizer for this problem is reverse‑waterfilling~\cite{BoydVandenberghe2004}:
\[
h_{t,i}=\min\{H_{t,i},\ \tau^\star_t\,w_{t,i}\},\qquad w_{t,i}=\rho(\lambda_{t,i}),
\]
with $\tau^\star_t$ set by $\sum_i e_{t,i}h_{t,i}=b_t$. 
This choice of $w$ minimizes $\delta_t$ among all weighted reverse‑waterfilling rules.
\end{proof}
...
Any weighted reverse‑waterfilling with weights $w$ admits the water‑level form
\[
  h^{(w)}_{T,i}(b)\;=\;\min\{\beta_i,\ \tau^{(w)}(b)\,w_i\},
\]
where $\tau^{(w)}(b)$ is chosen so that $\sum_i e_{T,i}\,h^{(w)}_{T,i}(b)=b$.
On an interval $[b_0,b_1]$ with fixed $A(b)$ we have
\[
  \frac{db}{d\tau^{(w)}}=\sum_{j\in A(b)} e_{T,j} w_j,
  \qquad
  \frac{d h^{(w)}_{T,i}}{db}
  \;=\;
  \begin{cases}
    \displaystyle \frac{w_i}{\sum_{j\in A(b)} e_{T,j} w_j},& i\in A(b),\\[1.0ex]
    0,& i\notin A(b).
  \end{cases}
\]
...
\[
  ...
  \;=\;
  \begin{cases}
    \displaystyle -\frac{\rho(\lambda_{T,i})\,e_{T,i}\,w_i}{\sum_{j\in A(b)} e_{T,j} w_j},& i\in A(b),\\[1.0ex]
    0,& i\notin A(b).
  \end{cases}
\]
For $\mathrm{RAP}(g^\star)$ we take $w_i=\rho(\lambda_{T,i})$, giving
\[
  \frac{d z^{(\mathrm{RAP})}_{T,i}}{db}
  \;=\;
  \begin{cases}
    \displaystyle -\frac{\rho(\lambda_{T,i})^2\,e_{T,i}}{\sum_{j\in A(b)} e_{T,j} \rho(\lambda_{T,j})},& i\in A(b),\\[1.0ex]
    0,& i\notin A(b).
  \end{cases}
\]
\emph{Step 3: Majorization at each budget.} On a fixed $A(b)$, sort indices by decreasing $\rho(\lambda_{T,i})$.
By the hypothesis that the haircut share vector of $h^{(w)}$ on $A(b)$,
\[
  \sigma^{(w)}_i(b)\ :=\ \frac{e_{T,i} w_i}{\sum_{j\in A(b)} e_{T,j} w_j},
\]
is no more concentrated on high‑$\rho$ indices than the RAP share
$\sigma^{(\mathrm{RAP})}_i(b)=\frac{e_{T,i} \rho(\lambda_{T,i})}{\sum_{j\in A(b)} e_{T,j} \rho(\lambda_{T,j})}$, the rearrangement/majorization inequality implies that for every $k$,
\[
  \sum_{i\le k}\!\frac{d z^{(\mathrm{RAP})}_{T,(i)}}{db}
  \;\leq\;
  \sum_{i\le k}\!\frac{d z^{(w)}_{T,(i)}}{db}
\]
where $(i)$ denotes the order by decreasing $\rho$.
Hence the instantaneous decrease vector under RAP weakly submajorizes that under $h^{(w)}$ on $[b_0,b_1]$.
