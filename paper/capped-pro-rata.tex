...
Fix time $T$, state $\mathcal{P}_n$, winners $\mathcal{W}_T$, and equities $e_i =  (e_T(\mathfrak{p}_i))_+$.
Let $s_{\pi, i} = (1-h_{\pi, i}) e_i$ be the surviving equity of position $i$ under ADL policy $\pi$.
We say that a feasible ADL rule $\pi$ (\S\ref{subsec:adl}) satisfies:
\begin{enumerate}
\item \emph{Sybil resistance:} Outcomes are invariant to splitting/merging accounts. For any split $e_i=\sum_{a=1}^r z_a$, the sum of survivors equals the original survivor: $\sum_{a} s^{(i\to z)}_{\pi,a} = s_{\pi,i}$.
\item \emph{Scale invariance:} $s_\pi(c e;\theta_\pi)=c\,s_\pi(e;\theta_\pi)$ for $c>0$.
\item \emph{Monotonicity:} If $e_1 \ge \dots \ge e_k$, then $s_{\pi,1}\ge\dots\ge s_{\pi,k}$.
\item \emph{Interior regularity:} The map $(e,H)\mapsto s_\pi(e;H)$ is $C^1$ on the interior, \ie~for $e_i>0$ for all $i$ and $0<H<\sum_i e_i$.
\end{enumerate}
...
If a feasible ADL policy $\pi$ satisfies the fairness properties, then $s_\pi(e;H)=s^{\mathrm{PR}}(e;H)$ for all feasible inputs.
\end{proposition}

\begin{proof}
Fix a feasible budget $H$ (\eg~maximum value of $\theta_n D_T$) and write $\beta_i\in(0,1]$ for the haircut cap on winner $i$ (as in the capped pro-rata rule~\eqref{eq:capped-pro-rata}), and sort so that $e_1\ge\cdots\ge e_k$.
...
On this interval, by scale invariance on $A$ (per–unit budget increases all active haircuts at the same rate) and feasibility,
\[
  \frac{d h_i}{d b} \;=\; \frac{1}{\sum_{j\in A} e_j}\quad (i\in A),
  \qquad
...
\[
  ...
  \;=\;
  \begin{cases}
    h_i(b_-)\;+\;\dfrac{b-b_-}{\sum_{j\in A} e_j} & (i\in A),\\[4pt]
    h_i(b_-) & (i\notin A).
  \end{cases}
\]
...
\[
  b \;=\; \sum_{i\notin A} e_i h_i(b_-)\;+\;\sum_{i\in A} e_i \min\{\eta_I(b),\beta_i\},
\]
...
In particular, on the \emph{interior} interval where no caps bind ($A=\{1,\dots,k\}$ and $b\in(0,W_T)$), we have $h_i(b)=\eta_I(b)$ for all $i$ so the budget identity reduces to
\[
  b \;=\; \sum_{i=1}^k e_i h_i(b) \;=\; \eta_I(b)\sum_{i=1}^k e_i \;=\; \eta_I(b)\,W_T,
\]
which implies $\eta_I(b)=b/W_T$.
Thus on this interval
$s_{\pi,i}=(1-\eta_I(b))e_i=(1-b/W_T)e_i$, i.e., proportional to equity.

Sybil resistance implies allocations depend only on total equity: splitting $e_i=\sum_a z_a$ leaves
...
At an endpoint $b_\ell$ where $\eta_I(b_\ell)$ first hits some cap $\beta_m$, all indices $j\ge m$ with $\beta_j\le \beta_m$ must saturate together; otherwise we would have $e_m\ge e_j$ but $(1-h_m(b_\ell))e_m < (1-h_j(b_\ell))e_j$, violating monotonicity.
Thus, as we pass from $I_\ell$ to $I_{\ell+1}$, a (possibly empty) tail $\{j>m\}$ leaves $A$, contributing a fixed amount $\sum_{j>m} e_j\beta_j$ to the budget, and the same water-filling argument applies on the remaining head with reduced budget.

Concatenating the solution across all intervals $I_\ell$ yields the reverse–waterfilling form
$h_i=\min\{\eta^\star,\beta_i\}$ with $\eta^\star$ chosen so that $\sum_i e_i h_i=H$, which is exactly capped pro–rata.
Uniqueness follows either from the strict convexity of the Euclidean projection onto
$\{h\in[0,1]^k:\ \sum_i e_i h_i=H\}$ or from the monotone one–dimensional search that defines $\eta^\star$.
\end{proof}
...
Fix time $T$, winners $\mathcal{W}_T$ with equities $e_i = e_T(\mathfrak{p}_i)_+$, and effective caps $\beta_i = \min\{\overline{h}_i, 1-\underline e_i/e_T(\mathfrak{p}_i)\}$ defined by the haircut and equity constraints~\eqref{eq:haircut-constraint}--\eqref{eq:equity-constraint}.
...
We consider choosing haircuts $h=(h_i)_{i\in\mathcal{W}_T}$ to minimize the equity-weighted total disutility $\sum_{i\in\mathcal{W}_T} e_i \phi(h_i)$ subject to the per-account bounds $0\le h_i\le\beta_i$ and the aggregate budget constraint $\sum_{i\in\mathcal{W}_T} e_i h_i = B_T$.
...
\begin{proposition}[Convex optimality]\label{prop:capped-prorata-optimal}
    ...
    \[
      \min_{h}\ \sum_{i\in\mathcal{W}_T} e_i\,\phi(h_i)
      \quad\text{s.t.}\quad
      \sum_{i\in\mathcal{W}_T} e_i h_i = B_T,\quad 0\le h_i\le \beta_i
    \]
    is the capped pro-rata rule~\eqref{eq:capped-pro-rata}, \ie
    \[
      h_{\pi_{CP},i}^\star = \min\{\eta^\star, \beta_i\},
    \]
    where $\eta^\star$ satisfies $\sum_{i\in\mathcal{W}_T} e_i h_{\pi_{CP},i}^\star = B_T$.
\end{proposition}
...
\begin{proof}
    ...
    For $B_T$ in the interior of the feasible region ($0<B_T<C(\beta)$, where $C(\beta)$ is defined in~\eqref{eq:max-cap}), Slater's condition holds, so KKT conditions are necessary and sufficient.
    The Lagrangian is
    \[
      \mathcal{L}(h,\lambda,\mu,\nu)
      \;=\;
      \sum_{i\in\mathcal{W}_T} e_i \phi(h_i)
      + \lambda\!\left(\sum_{i\in\mathcal{W}_T} e_i h_i - B_T\right)
    ...
    \]
    The budget constraint $\sum_{i\in\mathcal{W}_T} e_i h_i = B_T$ then reduces to finding $c$ such that
    \[
      \sum_{i\in\mathcal{W}_T} e_i \min\{c,\beta_i\} = B_T.
    \]
