...
The right tails of the winner equity distribution $\bar F_+(x)$ and loser shortfall distribution $\bar F_-(x)$ are regularly varying with indices $\alpha_+ > 0$ and $\alpha_- > 0$, respectively:
\[
...
\begin{enumerate}
    \item \emph{Aggregates:} $W_T/n \xrightarrow{p} \mu_+$, $D_T/n \xrightarrow{p} \mu_-$, and total fees $\Phi_T/n \xrightarrow{p} \mu_{\Phi}$ for constants $\mu_\pm, \mu_{\Phi} \in (0,\infty)$.
    \item \emph{Extremes:} The maximum winner equity $\omega_T$ and maximum loser shortfall $\Delta_T$ satisfy
    \[
        \frac{\omega_T}{b_{k_n}^+} \xrightarrow{p} c_+, \qquad \frac{\Delta_T}{b_{m_n}^-} \xrightarrow{p} c_-,
    \]
...
\begin{enumerate}
    ...
    \[
        c_{\text{lo}} \;\le\; \mathsf{PTSR}_T(\mathcal{P}_n,\pi_n) := \Expect\left[\frac{\omega_T^{\pi_n}}{D_T^{\pi_n}}\right] \;\le\; c_{\text{hi}};
    \]
    \item[(F2)] \emph{PMR stability:} There exist constants $0 < c'_{\text{lo}} \le c'_{\text{hi}} < \infty$ such that
    \[
        c'_{\text{lo}} \;\le\; \mathsf{PMR}_T(\mathcal{P}_n,\pi_n) := \Expect\left[\frac{\omega_T^{\pi_n}}{\Delta_T^{\pi_n}}\right] \;\le\; c'_{\text{hi}}.
    \]
\end{enumerate}
